...
def get_new_pokemon_url(prompt, num, type):
    url = 'https://qnezy3td8j.execute-api.us-east-1.amazonaws.com/beta/pkgen'
    if type == 0:
        inputs = "A ancient pokemon " + prompt
    else:
    ...
    # print(response.json())
    output_data = response.json()['body']
    return output_data
...
plot_df['percent'] = pokemon_df.groupby(['generation', 'status']).size().groupby(level=0).apply(
...
generation_plot3.update_layout(
    title_text='Evolution of Pokémon Total Points over Generations<br>(HP, Attack, Defense, Speed, Sp. Attack, Sp. Defense)',
    yaxis=dict(title=''),
    xaxis=dict(
        title='Generation',
        titlefont_size=16,
...
generation_plot3.update_traces(
    hovertemplate='Name:%{customdata[0]}<br>Status:%{customdata[1]}<br>Generation:%{x}<br>Total Points:%{y}')

generation_plot4 = px.scatter(pokemon_df, x='attack', y='defense', animation_frame='generation', size='hp',
                              color='type_1', hover_name='name',
...
generation_plot4.update_traces(
    hovertemplate='Name:%{customdata[0]}<br>Gen:%{customdata[1]}<br>Type:%{customdata[2]}<br>Attack:%{x}<br>Defense:%{y}')
...
app = dash.Dash(__name__, external_stylesheets=[dbc.themes.SKETCHY, external_stylesheets], server=server)
...
navbar = dbc.NavbarSimple(
        ...
        dbc.Row([
            ...
            dbc.Col([
                dbc.DropdownMenu([
                    ...
                    dbc.DropdownMenuItem("Shirley Luo",
                                         href='https://github.com/Shirley-Luo',
                                         target='_blank'),
    ...
    brand="Data Analytics: Deep Dive into Pokémon",
    color='info',
    fluid=True,
    dark=False,
...
tab1 = dbc.Tab(label="Pokémon Analysis🥳", tab_id='tab1',
               ...
               children=[
                   ...
                   dbc.Row([
                       ...
                       dbc.Col([
                           dls.Hash(
                               html.Div(id='pokemon-desc'), color='#435278', speed_multiplier=2, size=30,
                               fullscreen=False)
                       ]),
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='attributes-plot'), color='#435278', speed_multiplier=2, size=30,
                               fullscreen=False)
                       ])
                   ...
                   html.Div(html.Br()),
                   html.P(
                       "In the Pokémon evolution tree, the process of evolution will lead to a significant increase in the various attributes of the evolved Pokémon. " \
                   ...
                   html.P(
                       "Evolution usually results in higher base stats, which can make the evolved Pokémon more powerful and versatile in battle. " \
                       ...
                       style={"text-align": "left", 'font-size': '20'}),
                   html.P(
                       "Additionally, evolution sometimes changes the type of Pokémon, which has both advantages and disadvantages in terms of weaknesses and resistances. ",
                       style={"text-align": "left", 'font-size': '20'}),
                   html.Div(html.Br()),
                   dbc.Row([
                       dls.Hash(
                           html.Div(id='evolution-tree'), color='#435278', speed_multiplier=2, size=30,
                           fullscreen=False)
                   ]),
                   html.Div(html.Br()),
                   dbc.Button('Comparison Across Generations', color='secondary', size='lg', outline=False,
                              style={'font-size': '30px', 'width': '100%'}),
                   html.Div(html.Br()),
                   html.P(
                   ...
                   dbc.Row([
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='generation-plot1', figure=generation_plot1), color='#435278',
                               speed_multiplier=2, size=30, fullscreen=False)
                       ]),
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='generation-plot2', figure=generation_plot2), color='#435278',
                               speed_multiplier=2, size=30, fullscreen=False)
                   ...
                   dbc.Row([
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='generation-plot3', figure=generation_plot3), color='#435278',
                               speed_multiplier=2, size=30, fullscreen=False)
                       ]),
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='generation-plot4', figure=generation_plot4), color='#435278',
                               speed_multiplier=2, size=30, fullscreen=False)

                       ])
                   ]),
                   html.Div(html.Br()),
                   dbc.Button('Pokémon Similarity Based On Stats', color='secondary', size='lg', outline=False,
                              style={'font-size': '30px', 'width': '100%'}),
                   html.Div(html.Br()),
                   html.P(
                       "In our exploration of the vast Pokémon universe, we've employed K-means clustering and Principal Component Analysis to delve into the inherent similarities shared by these captivating creatures." \
                   ...
                   dbc.Row([
                       dbc.Col([
                           dls.Hash(
                               dcc.Graph(id='cluster-plot', figure=cluster_plot), color='#435278', speed_multiplier=2,
                               size=30, fullscreen=False)
                       ]),
                       dbc.Col([
                           html.Div([
                               html.P('Select a Pokémon ',
                                      style={'width': '30%', 'fontSize': 24, 'padding-left': '20px'}),
                               html.Div(
                                   dcc.Dropdown(
                           ...
                           dls.Hash(
                               dcc.Graph(id='similarity-plot'), color='#435278', speed_multiplier=2, size=30,
                               fullscreen=False)
...
def update(clicks, p1, p2,type):
    if clicks is None:
        content = html.Div(html.Img(src = 'https://pm1.aminoapps.com/6532/8441679e98967e38588e00e7a65f788ca0f820ee_00.jpg'),
                           className="d-grid gap-2 d-md-flex justify-content-md-center")
    else:
        type_num = 0 if type =='Primitive 🦕' else 1
        # print(type_num)
        pk_images = get_new_pokemon_url(f'fuse {p1.lower()} and {p2.lower}', 1, type_num)
...
def update(pokemon):
    if pokemon is None:
        ...
        fig = px.bar(bar_data, x='name', y=features, title=f"{pokemon} with Its Five Most Similar Pokémons",
